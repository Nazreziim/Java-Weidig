...
public class MainFrame extends JFrame implements FarbeundSchrift, ActionListener {
  private Font[] schrift = new Font[ 6 ];
  private JLabel status;
  private LabelmitMouseEvent schreibflaeche = new LabelmitMouseEvent();
  private CanvasmitMouseEvent malflaeche = new CanvasmitMouseEvent();
  ...
  public MainFrame() {
    ...
    schreibflaeche.setBorder( BorderFactory.createTitledBorder( "Schreibfläche anklicken" ) );
  ...
  public void actionPerformed( ActionEvent ae ) {
    ...
    for ( int i = 0; i < 6; i++ ) {
      if ( sBefehl.equals( nameSchrift[ i ] ) ) {
        status.setText( "Schrift geändert: " + schrift[ i ].getName() );
        // Schrift setzen
