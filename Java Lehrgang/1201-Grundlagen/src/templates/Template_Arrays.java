...
public class Template_Arrays {
	public static void main(String[] args) {
		...
		// Bitte nicht mit einer for-each Schleife befüllen - das führt zu
...
//			  }
		...
		// ^ sorgt für ArrayIndexOutOfBoundsException, da es immer am Ende ankommen wird
		...
		// Alles groß / alles klein mit Methode (warum hier?)
	...
	/*
	 * Diese Methode wandelt Groß- in Kleinbuchstaben um, oder umgekehrt. In welche
	 * Richtung die Umwandlung geschieht, hängt vom Parameter in_klein ab. Das
	 * Ergebnis der Umwandlung wird zurückgegeben.
	 */
	/**
	 * BEISPIEL FÜR <u>JAVA DOC</u> (HTML IST ERLAUBT)
	 * 
	 * @param buchstaben : Eingegebene Buchstaben
	 * @param in_klein   : Klein geschrieben? Wenn false, dann groß.
	 * @return DES GIBT A UMGWANDLTES CHAR-ARRAY ZRÜCK
	 */
	/*
	 * public static char[] umwandeln(char[] buchstaben, boolean in_klein) { int
	 * diff = Math.abs('A' - 'a');
	 * 
	 * for (int i = 0; i < buchstaben.length; i++) { if (in_klein) { // Groß in
	 * Kleinbuchstaben if ((buchstaben[i] >= 'A') && (buchstaben[i] <= 'Z')) {
	 * buchstaben[i] = (char) (buchstaben[i] + diff); } } else { // klein in
	 * Großbuchstaben if ((buchstaben[i] >= 'a') && (buchstaben[i] <= 'z')) {
