...
import static day2.p3.animation.tree.RandomUtil.getGaussianRandom;
import static day2.p3.animation.tree.RandomUtil.getRandom;
  ...
  public Branch( Branch parentBranch, Type type, int depth ) {
    ...
    switch ( type ) {
      ...
      case RIGHT : // create right branch
        transY = parentBranch.length
                - getGaussianRandom( 0, parentBranch.length, parentBranch.length / 10, parentBranch.length / 10 );
        locAngle.set( getGaussianRandom( 35, 10 ) * ( Type.LEFT == type ? 1 : -1 ) );
        if ( ( 0 > globalAngle.get() || globalAngle.get() > 180 ) && depth < 4 ) {
          length = parentBranch.length * getGaussianRandom( 0.3, 0.1 ); // branches pointed down are
                                                                        // shorter
        } else {
    ...
    getTransforms().add( new Rotate( locAngle.get(), 0, 0 ) ); // rotate branch to local angle
