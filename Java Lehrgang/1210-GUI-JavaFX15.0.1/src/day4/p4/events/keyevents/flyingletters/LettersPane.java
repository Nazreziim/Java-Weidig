...
public class LettersPane extends Region {
  ...
  private void createLetter( String c ) {
    ...
    final Interpolator interp = Interpolator.SPLINE( 0.295, 0.800, 0.305, 1.000 );
    final Timeline timeline = new Timeline();
    timeline.getKeyFrames().add( new KeyFrame( Duration.seconds( 3 ), ( ActionEvent event ) -> {
      // we are done remove us from scene
      getChildren().remove( letter );
    }, new KeyValue( letter.translateXProperty(), getRandom( 0.0f, getWidth() - letter.getBoundsInLocal().getWidth() ), interp ),
            new KeyValue( letter.translateYProperty(), getRandom( 0.0f, getHeight() - letter.getBoundsInLocal().getHeight() ),
                    interp ),
