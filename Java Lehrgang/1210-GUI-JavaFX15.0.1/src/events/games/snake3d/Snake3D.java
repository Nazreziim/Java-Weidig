...
public class Snake3D extends Application {
  ...
  private void onUpdate() {
    ...
    boolean collision = snake.getChildren().stream().map( n -> (Cube) n ).anyMatch( cube -> cube.isColliding( food ) );
  ...
  private static class Cube extends Box {
    ...
    public boolean isColliding( Cube c ) {
      return getTranslateX() == c.getTranslateX() && getTranslateY() == c.getTranslateY() && getTranslateZ() == c.getTranslateZ();
    }
  }
