public class Knoten {
	...
	public Knoten dropKnoten(int key) {
		...
		if (this.getKey() == key) { // Rekursionsende: zu l�schender Knoten ist this
			...
			else if (this.getLinks() == null) {
				erg = this.getRechts();
			} 
			else if (this.getRechts() == null) {
				erg = this.getLinks();
			}
							
			//3.) Knoten hat zwei Nachfolger
			else {	
				if (this.getLinks().hoehe() >= this.getRechts().hoehe()) {
					//kleinstes Element im rechten Teilbaum suchen:
					Knoten gr_links = groesster_links();
					// Den Wert des Elements in den key schreiben
					erg.setKey( gr_links.getKey() );
					// Durch Aufruf von dropKnoten, das �bernommene Element aus dem rechten Teilbaum l�schen
					erg.setLinks(this.getLinks().dropKnoten(gr_links.getKey()));	
				}else {
					//kleinstes Element im rechten Teilbaum suchen:
	...
	private Knoten kleinster_rechts() {
		...
		if (this.getRechts() == null) {
			...
			erg = this.getRechts().kleinster_rechts();
		}		
		return erg;
	}
	
	private Knoten groesster_links() {
		Knoten erg;
		if (this.getLinks() == null) {
			erg = this;
		} else {
			erg = this.getLinks().groesster_links();
		}		
		return erg;
	}
